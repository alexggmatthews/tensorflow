...
class CholeskyGradTest(tf.test.TestCase):
  ...
  def testOneBlockMatrices(self):
    ...
    self.runFiniteDifferences(shapes,dtypes=(tf.float32, tf.float64),scalarTest=True)      
      ...
      for shape in shapes:             
        for dtype in dtypes:
          if not(scalarTest):
            ...
            K = tf.matmul( x, tf.transpose(x) ) / shape[0] #This gives a positive definite matrix.
