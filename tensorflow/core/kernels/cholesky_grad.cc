 ...
 public:
  ...
  void CholeskyGradUnblocked(const ConstRef l_block, Ref grad_block) {
    ...
    for (int64 k = kMatrixSize-1; k >= 0; k--) {
      grad_block(k, k) -= (l_block
                       .block(k+1, k, kMatrixSize-(k+1), 1)
                       .adjoint()
                       *grad_block
                       .block(k+1, k, kMatrixSize-(k+1), 1))(0, 0)
                       /l_block(k, k);
      grad_block.block(k, k, kMatrixSize-k, 1) /= l_block(k, k);
      grad_block.block(k, 0, 1, k)
                -= grad_block.block(k, k, kMatrixSize-k, 1)
                .adjoint()
                *l_block.block(k, 0, kMatrixSize-k, k);
      grad_block.block(k+1, 0, kMatrixSize-(k+1), k)
                -= grad_block
                .block(k+1, k , kMatrixSize-(k+1), 1)
                *l_block.block(k, 0, 1, k);
      grad_block(k, k) *= 0.5;
    }
  }
};
